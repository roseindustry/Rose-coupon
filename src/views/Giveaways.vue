    ...
    methods: {
                    ...
                    this.giveaways = await Promise.all(
                        Object.keys(giveaways).map(async (key) => {
                            ...
                            if (subsSnapshot.exists()) {
                                ...
                                ).then((subscriptions) => subscriptions.filter((sub) => sub !== null)); // Filter out any null values
        ...
        async fetchFilteredGiveaways() {
            ...
            try {
                ...
                this.filteredGiveaways = this.giveaways.filter((giveaway) => {
                    return giveaway.subs && giveaway.subs.some((sub) => sub.subId === subId);
                });
        ...
        async createGiveaway() {
            try {
                ...
                if (this.imageFile) {
                    const sanitizedGiveawayName = this.giveaway.name.trim().toLowerCase().replace(/\s+/g, '-');
                    const fileExtension = this.imageFile.name.split('.').pop();
                    const fileName = `${sanitizedGiveawayName}-logo.${fileExtension}`;

                    const imageFileRef = storageRef(storage, `Events/${this.giveaway.name}/${fileName}`);
                ...
                if (this.selectedAffiliateIds.length > 0) {
                    for (let affiliateId of this.selectedAffiliateIds) {
                        const affiliateRef = dbRef(db, `Giveaways/${newGiveawayRef.key}/Affiliates/${affiliateId}`);
                ...
                if (this.selectedSubscriptionsIds.length > 0) {
                    for (let subId of this.selectedSubscriptionsIds) {
                        const subRef = dbRef(db, `Giveaways/${newGiveawayRef.key}/Subscriptions/${subId}`);
...
<template>
    ...
    <div v-if="this.role === 'admin'" class="container">
        <div class="d-flex justify-content-end align-items-center">
            <a href="#" class="btn btn-theme" data-bs-toggle="modal" data-bs-target="#createGiveawayModal"
                style="margin: 14px;" @click.prevent="fetchAffiliates(), fetchSubscriptions()">
        ...
        <div class="container-fluid">
            <div class="row">
                <div v-if="giveaways.length === 0" class="d-flex justify-content-center align-items-center">
                ...
                <div v-else>
                    <div class="row">
                        <div v-for="(giveaway, index) in giveaways" :key="giveaway.id"
                            ...
                            <div class="card h-100 position-relative shadow-sm">
                                ...
                                <div class="img-container position-relative">
                                    <div v-if="!giveaway.isEditing" class="img"
                                        :style="{ backgroundImage: 'url(' + giveaway.image + ')', backgroundSize: 'cover', backgroundPosition: 'center', height: '200px' }">
                                    </div>
                                    <div v-if="giveaway.isEditing">
                                        ...
                                        <input type="file" @change="event => previewUpdatedImage(event, giveaway)"
                                            class="form-control" />
                                    </div>
                                </div>
                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <p class="card-text"><strong>Fecha: </strong>{{ formatDate(giveaway.date) }}</p>
                                    ...
                                    <div class="row">
                                        <div v-for="affiliate in giveaway.affiliates" :key="affiliate.id"
                                            class="col-6 mb-3">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="createGiveawayModalLabel">Crear Sorteo</h5>
                        ...
                        <div class="container">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-12 mb-3">
                                    <label for="giveawayAffiliate" class="form-label">Comercios Afiliados para el
                                        Sorteo</label>
                                    <div class="dropdown">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-12 mb-3">
                                    <label for="giveawaySubscriptions" class="form-label">Suscripciones
                                        permitidas</label>
                                    <div class="dropdown">
                                        <button class="btn btn-secondary dropdown-toggle" type="button"
                                            id="dropdownSubscriptions" data-bs-toggle="dropdown"
                                            data-bs-auto-close="false" aria-expanded="false">
                                            {{ selectedSubscriptionsIds.length > 0 ? selectedSubscriptionsIds.map(id =>
                                                subscriptions.find(sub => sub.id === id).name.toUpperCase()).join(', ') :
                                                'Seleccione...' }}
                                        </button>
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li v-if="subscriptions.length === 0">
                                                <p style="margin: 10px;">No hay suscripciones registradas.</p>
                                            </li>
                                            <li v-for="sub in subscriptions" :key="sub.id">
                                                <div class="form-check" style="margin: 10px;">
                                                    <input type="checkbox" class="form-check-input"
                                                        :id="'dropdownCheck_' + sub.id" :value="sub.id"
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-12 mb-3">
                                    ...
                                    <input type="date" v-model="giveaway.date" class="form-control"
                                        style="width: auto;" />
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-12 mb-3 d-flex align-items-center">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="giveawayStatus"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-theme" @click="createGiveaway()"
    ...
    <div v-if="this.role === 'cliente'" class="container">
        <div class="container-fluid">
            <div class="row">
                <div v-if="filteredGiveaways.length === 0" class="d-flex justify-content-center align-items-center"
                    style="height: 100vh;">
                    <div class="text-center">
                        <div class="mb-3 mt-n5">
                            <i class="fa-solid fa-gifts text-body text-opacity-25" style="font-size: 5em"></i>
                        </div>
                        <h5>No hay Sorteos registrados para clientes con tu nivel de suscripción.</h5>
                    </div>
                </div>
                <div v-else>
                    <!-- Loop through giveaways and display them in cards -->
                    <div class="row">
                        <div v-for="giveaway in filteredGiveaways" :key="giveaway.id"
                            class="col-12 col-sm-6 col-md-4 mb-4">
